...
Fonctionnalités multi-localisés dans Office 365 activer un seul client de couvrir plusieurs emplacements géographiques. Lorsque multi-geo est activée, les clients pouvant sélectionner l’emplacement du contenu des boîtes aux lettres Exchange Online (données au repos) par utilisateur.

Votre emplacement initiale du client (désigné comme emplacement central) est déterminée en fonction de votre adresse de facturation. Lorsque multi-geo est activé, vous pouvez placer des boîtes aux lettres dans les emplacements satellites supplémentaires par :

- Création d’une nouvelle boîte aux lettres Exchange Online directement dans un emplacement satellites.

- Déplacement d’une boîte aux lettres Exchange Online existant dans un emplacement satellites.

- Intégration d’une boîte aux lettres d’une organisation d’Exchange sur site directement dans un emplacement satellites.

Geo aux emplacements suivants sont disponibles pour une utilisation dans une configuration Multi-localisés :
...
Avant de pouvoir commencer à l’aide de fonctionnalités Multi-localisés dans Exchange Online, Microsoft a besoin configurer votre client pour la prise en charge multi-geo Exchange Online. Ce processus de configuration unique est déclenché une fois que vous commandez Qu'office 365 Multi-localisés et les licences s’affichent-ils dans votre client. Ce processus de configuration unique adopter généralement moins de 30 jours. Pour Office 365 Multi-localisés, contactez votre représentant Microsoft. Pour plus d’informations, voir https://aka.ms/Multi-Geo.
...
Exchange Online synchronise la propriété **PreferredDataLocation** d’Azure Active Directory dans la propriété **MailboxRegion** dans le service d’annuaire Exchange Online. La valeur de **MailboxRegion** détermine le géographique où les boîtes aux lettres utilisateur et des boîtes aux lettres d’archive associé seront placées. Il n’est pas possible de configurer principales boîte aux lettres et archivage boîtes aux lettres un utilisateur pour résident dans des emplacements différents localisés. Qu’un seul emplacement géographique peut être configuré par l’objet utilisateur.

- Lorsque **PreferredDataLocation** est configuré sur un utilisateur disposant d’une boîte aux lettres existante, la boîte aux lettres est de placer dans une file d’attente de déplacement et automatiquement déplacés vers l’emplacement géographique spécifié. 

- Lorsque **PreferredDataLocation** est configuré sur un utilisateur sans une boîte aux lettres existante, la boîte aux lettres système mis en service à l’emplacement géographique spécifié. 

- Lorsque **PreferredDataLocation** sur un utilisateur n’est pas spécifié, la boîte aux lettres est placé dans le site central.

- Si le code **PreferredDataLocation** est incorrect (par exemple, un type de NAN au lieu du nom), la boîte aux lettres est placé dans le site central.

**Remarque**: fonctionnalités multi-geo et Skype pour les réunions en ligne Business régional hébergée par les deux permet de la propriété **PreferredDataLocation** sur les objets utilisateur localisent les services. Si vous configurez des valeurs **PreferredDataLocation** sur les objets utilisateur pour les réunions régional hébergées, la boîte aux lettres pour ces utilisateurs est automatiquement déplacée vers l’emplacement géographique spécifiée après que multi-geo est activée sur le client Office 365.
...
En règle générale, Exchange Online PowerShell se connecte à l’emplacement de géolocalisation par défaut. Mais, vous pouvez également vous connecter directement à des emplacements par défaut non localisés. En raison des améliorations des performances, nous vous recommandons de connexion directement à l’emplacement géographique non par défaut lorsque vous gérez uniquement les utilisateurs de cet emplacement géographique.
...
Propriétés de boîtes aux lettres associées à l’applet de commande **Get-Mailbox** dans Exchange Online PowerShell affiche le multi-geo suivante :
...
- **MailboxRegion**: Spécifie le code d’emplacement géographique qui a été défini par l’administrateur (synchronisée à partir de **PreferredDataLocation** dans Azure Active Directory).
...
> **Remarque :** Si le code d’emplacement géographique dans le nom de base de données ne correspond pas à la valeur de **MailboxRegion** , la boîte aux lettres sera automatiquement à placer dans une file d’attente de déplacement et déplacée vers l’emplacement géographique spécifié par la valeur **MailboxRegion** (Exchange Online cherche une non-concordance entre ces valeurs de propriété).
...
- La durée nécessaire pour déplacer un mailboxfrom que son geo actuel vers le nouvel emplacement géographique de votre choix dépend de plusieurs facteurs :
...
3. Supprimer la licence de la boîte aux lettres après que qu’il a été déplacée vers l’emplacement géographique sélectionné à replacer dans un état désactivé.

### <a name="create-new-cloud-mailboxes-in-a-specific-geo"></a>Créer des nouvelles boîtes aux lettres en nuage dans une région 
Pour créer une nouvelle boîte aux lettres dans un emplacement géographique spécifique, vous devez effectuer une de ces étapes :
...
5. Répétez l’étape 4 # pour chaque boîte aux lettres à migrer à partir d’Exchange en local à l’emplacement de satellites que vous êtes actuellement connecté.

6. Si vous avez besoin migrer des boîtes aux lettres supplémentaires à un emplacement différent satellites, répétez les étapes 2 à 4 pour chaque emplacement satellites spécifiques.

### <a name="multi-geo-reporting"></a>Création de rapports multi-localisés
**Rapports d’utilisation Multi-localisés** dans le centre d’administration Office 365 affiche le nombre d’utilisateurs par emplacement géographique. Le rapport affiche la distribution des utilisateurs pour le mois en cours et fournit des données d’historique pour les 6 derniers mois.
