 ...
 **Résumé :** Comprendre pourquoi vous devez utiliser Office 365 PowerShell pour gérer Office 365, dans certains cas plus efficacement et dans d’autres cas par nécessité.
...
- Office 365 dispose de fonctionnalités que vous pouvez configurer uniquement à l'aide d' Office 365 PowerShell
...
> [!NOTE]
> Cette commande exige que vous installiez le [module Windows Azure Active Directory](https://technet.microsoft.com/en-us/library/jj151815.aspx). 
...
> [!TIP]
>  L’interprétation de cette commande d’Office 365 PowerShell est : obtenir tous les utilisateurs dans l’abonnement à Office 365 actuelle ( **Get-MsolUser** ), mais n’afficher que le nom et l’emplacement de chaque utilisateur ( **Sélectionnez un DisplayName, UsageLocation** ).
...
> [!TIP]
>  L’interprétation de cette commande d’Office 365 PowerShell est : obtenir tous les utilisateurs dans l’abonnement à Office 365 actuelle, mais uniquement d’afficher le nom et l’emplacement de chaque utilisateur et de les trier d’abord par leur emplacement, puis par leur nom ( **UsageLocation de tri, DisplayName** ).
  
Vous pouvez également employer un filtrage supplémentaire. Par exemple, si vous voulez uniquement voir les informations relatives aux utilisateurs basés au Brésil, utilisez cette commande :
...
> [!TIP]
>  L’interprétation de cette commande d’Office 365 PowerShell est : obtenir tous les utilisateurs dans l’abonnement à Office 365 en cours dont l’emplacement est le Brésil ( **où {$\_. UsageLocation - eq « BR »}** ), puis afficher le nom et l’emplacement de chaque utilisateur.
...
L’interprétation de cet ensemble de commandes d’Office 365 PowerShell est la suivante :
- Obtenir tous les utilisateurs dans l’abonnement à Office 365 actuelle et de stocker les informations dans une variable nommée $x ( **$x = Get-MsolUser** ).
...
## <a name="office-365-has-features-that-you-can-only-configure-with-office-365-powershell"></a>Office 365 dispose de fonctionnalités que vous pouvez configurer uniquement avec Office 365 PowerShell
<a name="only"> </a>
...
> [!TIP]
>  L’interprétation de cette commande d’Office 365 PowerShell est : pour les paramètres à nouveau Skype pour les réunions en ligne de Business ( **Set-CsMeetingConfiguration** ), désactiver permettant aux utilisateurs anonymes de gain d’entrée automatique à des réunions ( **- AdmitAnonymousUsersByDefault $False** ), désactivez la possibilité pour les participants aux réunions d’enregistrement ( **AllowConferenceRecording - $False** ) et ne désignez pas de tous les utilisateurs de votre organisation en tant que présentateurs ( **- DesignateAsPresenter « None »** ).
...
> [!NOTE]
> Cette commande exige que vous installiez la [Connexion à SharePoint Online PowerShell](https://technet.microsoft.com/library/fp161372.aspx). 
  
> [!TIP]
>  L’interprétation de cette commande d’Office 365 PowerShell est : obtenir tous les sites SharePoint dans l’abonnement à Office 365 actuelle ( **Get-SPOSite** ) et pour chaque site, supprimez Ken Meyer dans la liste des utilisateurs qui peuvent y accéder ( **ForEach {Remove-SPOUser - Site $\_. URL - LoginName « kenmyer@litwareinc.com »}** ).
...
> [!TIP]
>  L’interprétation de cette commande d’Office 365 PowerShell est : obtenir tous les sites SharePoint dans l’abonnement à Office 365 en cours et pour chaque site, autoriser l’accès de Bonnie Kearney en ajoutant son nom de connexion pour le groupe de membres du site ( **, ForEach {Add-SPOUser-Site $\_. URL - LoginName « bkearney@litwareinc.com »-groupe « Membres »}** ).
...
> [!TIP]
>  L’interprétation de cette commande d’Office 365 PowerShell est : obtenir tous les utilisateurs dans l’abonnement à Office 365 actuelle qui ont une boîte aux lettres dans les villes de San Diego ou Bloomington ( **où {$\_. RecipientTypeDetails - eq « UserMailbox »- et ($\_. Ville - eq « San Diego »- ou $\_. Ville - eq "Bloomington")}** ), puis afficher le nom et la ville de chaque ( **Sélectionnez un DisplayName, ville** ).
...
> [!TIP]
>  L’interprétation de cette commande d’Office 365 PowerShell est : obtenir tous les utilisateurs dans l’abonnement à Office 365 actuelle qui ont une boîte aux lettres ne se trouve ne pas dans la ville de Bloomington ( **où {$\_. RecipientTypeDetails - eq « UserMailbox »- et $\_. Ville - ne « Bloomington »}** ), puis afficher le nom et la ville de chacun.
...
> [!TIP]
>  L’interprétation de cette commande d’Office 365 PowerShell est : obtenir tous les utilisateurs dans l’abonnement à Office 365 actuelle, mais utiliser un filtre qui ne répertorie que les utilisateurs dont le nom se termine par « son » ( **-filtre ' {nom-comme «\*fils »}'** ). Le \* est l’acronyme de n’importe quel jeu de caractères, qui sont des lettres pour le nom de l’utilisateur.
...
> [!TIP]
>  L’interprétation de cette commande d’Office 365 PowerShell est : obtenir tous le Skype pour des utilisateurs professionnels en ligne dans l’abonnement à Office 365 actuelle ( **Get-CsOnlineUser** ), d’obtenir uniquement le nom d’utilisateur, l’UPN et l’emplacement ( **Sélectionnez DisplayName, UserPrincipalName, UsageLocation** ), puis enregistrez que les informations contenues dans le fichier CSV nommé C:\\journaux\\SfBUsers.csv ( **Export-Csv-chemin d’accès « C:\\journaux\\SfBUsers.csv » - NoTypeInformation** ).
...
> [!TIP]
>  L’interprétation de cette commande d’Office 365 PowerShell est : obtenir tous le Skype pour obtenir uniquement le nom d’utilisateur, l’UPN et l’emplacement des utilisateurs professionnels en ligne dans l’abonnement à Office 365 actuelle et puis envoyer ces informations à l’imprimante de Windows par défaut ( ** Out-Printer** ).
  
Le document imprimé est le même simple de mise en forme en tant que l’affichage dans la fenêtre de commande PowerShell de Office 365, mais une fois que vous avez créé une commande d’Office 365 PowerShell pour répertorier ce dont vous avez besoin, vous ajoutez simplement **| Out-imprimante** à la fin de la commande pour obtenir une copie papier de travailler à partir de.
...
- Obtenir tous les utilisateurs dans l’abonnement à Office 365 actuelle et de stocker les informations dans une variable nommée $x ( **$x = Get-MsolUser** ).
- Démarrer une boucle qui s’exécute sur tous les utilisateurs dans la variable nommée $x ( **foreach ($i dans $x)** ).  
- Définir une variable nommée $y et stocker des informations de boîte aux lettres de l’utilisateur qu’il contient ( **$y = Get-Mailbox-Identity $i.UserPrincipalName** ).
- Ajouter une nouvelle propriété aux informations utilisateur nommées IsMailBoxEnabled et lui affecter la valeur de la propriété IsMailBoxEnabled de la boîte aux lettres de l’utilisateur ( **$i | Add-Member - MemberType NoteProperty-Name IsMailboxEnabled-valeur $y.IsMailboxEnabled** ).
- Définir une variable nommée $y et y stockez Skype de l’utilisateur pour les informations commerciales en ligne ( **$y = Get-CsOnlineUser-Identity $i.UserPrincipalName** ).
- Ajouter une nouvelle propriété aux informations utilisateur nommées EnabledForSfB et lui affecter la valeur de la propriété Enabled de Skype de l’utilisateur pour les informations commerciales en ligne ( **$i | Add-Member - MemberType NoteProperty-Name EnabledForSfB-valeur $y.Enabled** ).
- Afficher la liste des utilisateurs, mais inclure uniquement leur nom, s’ils sont sous licence et les deux nouvelles propriétés qui indiquent si leur boîte aux lettres est activée et si elles sont activées pour Skype pour Business Online ( **$x | Sélectionnez DisplayName, IsLicensed, IsMailboxEnabled, EnabledforSfB** ).
