...
4. À partir de la boîte de dialogue **Modifier la liaison de Site** , vérifiez le nom d’hôte est le même que le nom convivial, comme illustré dans l’image suivante.<br/>![Modification de la liaison de site dans IIS](media/SAML11/fig4-editsitebinding.png)<br/>
...
5. Cliquez sur le lien **configurer (nom de l’application)** pour accéder à la page Configurer la session.<br/>![Configuration d’une authentification unique sur la page](media/SAML11/fig7-configssopage.png)<br/> 
...
6.  Naviguez vers le volet des **Propriétés** de l’application. Copiez et collez la valeur de l’ID de l’objet dans la table que vous avez configuré à l’étape 3.<br/>![Volet des propriétés de l’application](media/SAML11/fig8-propertiespane.png)<br/>
...
> [!IMPORTANT]
> Il est important de suivre toutes les étapes, y compris la définition de la connexion personnalisée dans la page « /_trust/ » comme indiqué. La configuration ne fonctionnera pas correctement, sauf si toutes les étapes sont suivies.
...
4. Dans la stratégie d’Application Web, cliquez sur **Ajouter des utilisateurs**.<br/>![Recherche d’un utilisateur par leur nom de revendication](media/SAML11/fig11-searchbynameclaim.png)<br/>
...
9. Dans autorisations, cliquez sur **Contrôle total**.<br/>![L’autorisation contrôle total à un utilisateur basée sur les revendications](media/SAML11/fig12-grantfullcontrol.png)<br/>
...
Lorsque l’application Azure AD est créée dans le portail, elle utilise par défaut SAML 2.0. SharePoint Server 2016 requiert que le format de jeton SAML 1.1. Le script suivant supprime la stratégie de SAML 2.0 par défaut et ajouter une nouvelle stratégie aux jetons SAML 1.1 de problème. 

> Ce code nécessite le téléchargement de l' accompagnement [exemples illustrant l’interaction avec Azure Active Directory Graph](https://github.com/kaevans/spsaml11/tree/master/scripts). Si vous téléchargez les scripts dans un fichier ZIP à partir de référentiels à un ordinateur de bureau Windows, veillez à débloquer le `MSGraphTokenLifetimePolicy.psm1` fichier de module de script et les `Initialize.ps1` fichier de script (cliquez sur Propriétés, cliquez sur débloquer, cliquez sur OK). ![Déblocage des fichiers téléchargés](media/SAML11/fig17-unblock.png)

Une fois que l’exemple de script est téléchargé, créer un nouveau script PowerShell utilisant le code suivant, en remplaçant l’espace réservé par le chemin d’accès de fichier des fichiers `Initialize.ps1` sur votre ordinateur local. Remplacez l’espace réservé ID d’objet application avec l’ID objet d’application que vous avez entré dans le tableau 1. Une fois créée, exécutez le script PowerShell. 
...
4. Ajouter l’URL de l’application web supplémentaires avec `/_trust/default.aspx` ajoutée à l’URL (tel que `https://sales.contoso.local/_trust/default.aspx`) et cliquez sur **Enregistrer**. 
